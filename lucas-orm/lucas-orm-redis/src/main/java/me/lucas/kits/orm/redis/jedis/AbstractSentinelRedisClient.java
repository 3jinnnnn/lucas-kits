...
@Slf4j
@NoArgsConstructor
public abstract class AbstractSentinelRedisClient extends AbstractRefreshCacheScript implements RedisClient {
    ...
    public void close(final Jedis shardedJedis) {
     ...
     *
     * @see JSON#toJSONString(Object, SerializerFeature...)
     */
     ...
     *
     * @see TypeReference
     * @see JSON#parseObject(String, TypeReference, com.alibaba.fastjson.parser.Feature...)
     */
        ...
        for (final String attribute : attributes) {
            if (!StringUtils.isEmpty(StringUtils.trim(attribute)) && !StringUtils.startsWith(attribute, "#")) {
                ...
                if (keyvalue.length == 2) {
                    ...
                    final String value = StringUtils.endsWith(keyvalue[1], "\r") ? StringUtils
                            .substring(keyvalue[1], 0, keyvalue[1].length() - 1) : keyvalue[1];
    ...
    public <T> ScanResult<T> scan(final long cursor, final ScanParams params, final TypeReference<T> type) {
        ...
        final List<T> newValues = values.stream().map(value -> JSON.parseObject(value, type))
    ...
    @Override
    public <T> ScanResult<Entry<String, T>> hscan(final String key, long cursor, final ScanParams params,
        ...
        entrys.forEach(entry -> newEntrys
                .add(new AbstractMap.SimpleEntry<>(entry.getKey(), JSON.parseObject(entry.getValue(), type))));
    ...
    @Override
    public <T> Map<String, T> bpop(final String[] keys, final int timeout, final Mark pop,
            final TypeReference<T> type) {
    ...
    @Override
    public <T> T brpoplpush(final String source, final String destination, final int timeout,
            final TypeReference<T> type) {
    ...
    @Override
    public boolean push(final String key, final String scanKey, final Object value, final Mark push,
            final Mark policy) {
    ...
    public long sreplace(final String key, final Collection<Object> oldMembers, final Collection<Object> newMembers) {
        final Object[] olds = oldMembers == null || oldMembers.size() == 0
                ? new Object[0]
                : oldMembers.toArray(new Object[oldMembers.size()]);
        final Object[] news = newMembers == null || newMembers.size() == 0
                ? new Object[0]
                : newMembers.toArray(new Object[newMembers.size()]);
    ...
    @Override
    public <T> ScanResult<T> sscan(final String key, final long cursor, final ScanParams params,
            final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrangeByLex(final String key, final String min, final String max, final int offset,
            final int count, final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrangeByScore(final String key, final double min, final double max, final int offset,
            final int count, final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrangeByScore(final String key, final String min, final String max, final int offset,
            final int count, final TypeReference<T> type) {
    ...
    @Override
    public <T> Map<T, Double> zrangeByScoreWithScores(final String key, final double min, final double max,
            final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrevrangeByLex(final String key, final String max, final String min,
            final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrevrangeByLex(final String key, final String max, final String min, final int offset,
            final int count, final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrevrangeByScore(final String key, final double max, final double min,
            final TypeReference<T> type) {
    ...
    @Override
    public <T> Set<T> zrevrangeByScore(final String key, final double max, final double min, final int offset,
            final int count, final TypeReference<T> type) {
    ...
    @Override
    public <T> ScanResult<Entry<T, Double>> zscan(final String key, final long cursor, final ScanParams params,
            final TypeReference<T> type) {
