...
local result
local compareCacheKey = 'LUA_COMPARE_CACHE@KEY[' .. KEYS[1] .. ']FIELD[' .. KEYS[2] .. ']'
-- 判断key是否存在
local exist = redis.call('hexists', KEYS[1], KEYS[2])

if exist == 0 then
    -- key不存在直接写入缓存
    result = redis.call('hset', KEYS[1], KEYS[2], ARGV[1])
